...
| Los archivos originales subidos (`out/uploads/<uploadId>/raw/`) quedan en disco sin política de expiración ni límite de cuota; en Vercel caen en `/tmp` (efímero por invocación), pero en un servidor persistente se acumularían indefinidamente. | Añadir una tarea de limpieza por antigüedad y un límite de cuota por sesión antes de exponer la carga de documentos en producción. |
